...
try {
    ...
    $old_data = db_select_single($koneksi, "SELECT foto_profile FROM user_rt WHERE sk_rt = ?", "s", [$sk_rt]);
    ...
    // Update database
    $update_result = db_update($koneksi, "UPDATE user_rt SET foto_profile = ? WHERE sk_rt = ?", "ss", [$new_filename, $sk_rt]);
    ...
    // Update session data
    $_SESSION['user_rt']['foto_profile'] = $new_filename;
